...
type AvsWriterer interface {
	...
	) (cstaskmanager.IAnzenTaskManagerOraclePullTask, uint32, error)
	SendNewTaskNumberToSquare(
		ctx context.Context,
		numToSquare *big.Int,
		quorumThresholdPercentage sdktypes.QuorumThresholdPercentage,
		quorumNumbers sdktypes.QuorumNums,
	) (cstaskmanager.IAnzenTaskManagerTask, uint32, error)
	RaiseChallenge(
		ctx context.Context,
...
func (w *AvsWriter) SendNewOraclePullTask(
	...
	if err != nil {
		...
		return cstaskmanager.IAnzenTaskManagerOraclePullTask{}, 0, err
	}
...
func (w *AvsWriter) SendNewTaskNumberToSquare(ctx context.Context, numToSquare *big.Int, quorumThresholdPercentage sdktypes.QuorumThresholdPercentage, quorumNumbers sdktypes.QuorumNums) (cstaskmanager.IAnzenTaskManagerTask, uint32, error) {
	txOpts, err := w.TxMgr.GetNoSendTxOpts()
	if err != nil {
		w.logger.Errorf("Error getting tx opts")
		return cstaskmanager.IAnzenTaskManagerTask{}, 0, err
	}
	...
	newTaskCreatedEvent, err := w.AvsContractBindings.TaskManager.ContractAnzenTaskManagerFilterer.ParseNewTaskCreated(*receipt.Logs[0])
	if err != nil {
...
func (w *AvsWriter) SendAggregatedOracleResponse(
	ctx context.Context, task cstaskmanager.IAnzenTaskManagerOraclePullTask,
	taskResponse cstaskmanager.IAnzenTaskManagerOraclePullTaskResponse,
	nonSignerStakesAndSignature cstaskmanager.IBLSSignatureCheckerNonSignerStakesAndSignature,
) (*types.Receipt, error) {
	txOpts, err := w.TxMgr.GetNoSendTxOpts()
